...
void CreateNewCommand(void) {
    ...
    (grp[grpcount-1].command + grp[grpcount-1].commandcount - 1)->type = COMMAND;
}
...
// returns 1 if char is '<', otherwise returns 0
...
void Put2NewWord(char ch) {
    ...
    if (ch == 0) {                               // beginning of new empty word (only for quote)
        currcomm->cmds[currcomm->cmdslen-1] = malloc(sizeof(char));
        currcomm->cmds[currcomm->cmdslen-1][0] = '\0';
    }
    else {                                       // beginning of new word with 1 char
...
void Parser(char ch) {
    switch (state) {
        case SPACE:
            ...
            else if ( Is_redir_in(ch) ) {
                redir_type = 3;
                state = REDIR_IN;
            }
            else if ( Is_redir_out(ch) ) {
                redir_type = 1;
                state = REDIR_OUT;
            }
            else { // is regular.
        ...
        case REDIR_IN:
            if ( Is_space(ch) ) {
                ...
                CreateNewCommand();
                (grp[grpcount-1].command + grp[grpcount-1].commandcount - 1)->type = READ_FILE;
                Put2NewWord(0);
                state = QUOTE;
            }
            else {
        ...
        case REDIR_OUT:
            if ( Is_space(ch) ) {
                ...
                CreateNewCommand();
                if (redir_type == 1) {
                    (grp[grpcount-1].command + grp[grpcount-1].commandcount - 1)->type = WRITE_FILE;
                }
                else {
                    (grp[grpcount-1].command + grp[grpcount-1].commandcount - 1)->type = APPEND_FILE;
                }
                Put2NewWord(0);
                state = QUOTE;
            }
...
int ExecuteSingleCommand(int k, int l, int bckgrnd_mode, int read_fd, int write_fd, int prs_list_number) {
    if (grp[k].command[l].type != COMMAND) {
        return 0;
    }
    ...
    for (int i = 0; i < grp[k].commandcount; i++) {
        cmnd* cmd = &grp[k].command[i];
        
        if (cmd->type == READ_FILE && cmd->cmdslen > 0) {
            input_fd = open(cmd->cmds[0], O_RDONLY);
            if (input_fd == -1) {
        ...
        else if (cmd->type == WRITE_FILE && cmd->cmdslen > 0) {
            output_fd = open(cmd->cmds[0], O_WRONLY | O_CREAT | O_TRUNC, 0644);
            if (output_fd == -1) {
        ...
        else if (cmd->type == APPEND_FILE && cmd->cmdslen > 0) {
            output_fd = open(cmd->cmds[0], O_WRONLY | O_CREAT | O_APPEND, 0644);
            if (output_fd == -1) {
                perror("open output file");
                if (input_fd != -1) close(input_fd);
                return 1;
    ...
    char *s;
    if (pid == 0) {                              // SON
        if (input_redirected && input_fd != -1) {
        ...
        if (output_redirected && output_fd != -1) {
...
void Debug(void) {
    for (int i=0; i<grpcount; ++i) {
        ...
        for (int j=0; j<grp[i].commandcount; ++j) {
            printf("     COMMAND %d (type: %d):\n", j, grp[i].command[j].type);fflush(stdout);
